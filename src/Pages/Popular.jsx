...
const Popular = () => {
  ...
  useEffect(() => {
    const fetchPopularAnime = async () => {
      try {
        const response = await api.get(`https://api.jikan.moe/v4/top/anime?sfw=true&filter=bypopularity&page=${page}`);
  ...
  return (
    <>
      <Helmet>
        <title>MaNime - Populer Anime</title>
        <meta property="og:url" content="https://manime-reactjs.vercel.app/popular" />
        <meta property="og:title" content="MaNime - Populer Anime" />
        <meta property="og:description" content="Daftar anime populer yang diurutkan berdasarkan jumlah penggemar." />
        <meta property="og:image" content="https://manime-reactjs.vercel.app/img/MaNime-white.png" />
        <meta property="og:image:type" content="image/png" />
        <link rel="canonical" href="https://manime-reactjs.vercel.app/popular" />
      </Helmet>
      <Header />
      <section className="popular-anime" id="popular">
        <h1>Populer Anime</h1>
        <div className="popular-anime2-container">
          {loading
            ? Array.from({ length: popularAnime.length > 0 ? popularAnime.length : 25 }).map((_, index) => (
                <div className="popular-anime2-card" key={index}>
                  <Skeleton height={300} width="100%" />
                </div>
              ))
            : popularAnime.map((anime, index) => (
                <div className="popular-anime2-card" key={`${anime.mal_id}${index}`}>
                  <Link
                    to={`/detail/${anime.mal_id}/${anime.title
                      ...
                      .replace(/\s+/g, "_")}`}
                  >
                    <img src={anime.images.webp.large_image_url} alt={anime.title} className="popular-anime2-image" loading="lazy" />
                    <div className="popular-anime2-detail">
