...
const PopNow = () => {
  ...
  useEffect(() => {
    const fetchPopNow = async () => {
      try {
        const response = await api.get(`https://api.jikan.moe/v4/anime?sfw=true&status=airing&min_score=7&order_by=score&sort=desc&page=${page}`);
        setPopNow(response.data.data);
  ...
  return (
    <>
      <Helmet>
        <title>MaNime - Populer Musim Ini</title>
        <meta property="og:url" content="https://manime-reactjs.vercel.app/popnow" />
        <meta property="og:title" content="MaNime - Populer Musim Ini" />
        <meta property="og:description" content="Daftar anime yang tayang musim ini yang diurutkan berdasarkan jumlah score atau rating yang didapatkan." />
        <meta property="og:image" content="https://manime-reactjs.vercel.app/img/MaNime-white.png" />
        <meta property="og:image:type" content="image/png" />
        <link rel="canonical" href="https://manime-reactjs.vercel.app/popnow" />
      </Helmet>
      <Header />
      <section className="popular-now" id="popnow">
        <h1>Populer Musim Ini</h1>
        <div className="popular-now2-container">
          {loading
            ? Array.from({ length: popNow.length > 0 ? popNow.length : 25 }).map((_, index) => (
                <div className="popular-now2-card" key={index}>
                  <Skeleton height={300} width="100%" />
                </div>
              ))
            : popNow.map((anime, index) => (
                <div className="popular-now2-card" key={`${anime.mal_id}${index}`}>
                  <Link
                    to={`/detail/${anime.mal_id}/${anime.title
                      ...
                      .replace(/\s+/g, "_")}`}
                  >
                    <img src={anime.images.webp.large_image_url} alt={anime.title} className="popular-now2-image" loading="lazy" />
                    <div className="popular-now2-detail">
