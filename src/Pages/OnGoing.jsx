...
const OnGoing = () => {
  ...
  useEffect(() => {
    const fetchOnGoingAnime = async () => {
      try {
        const response = await api.get(`https://api.jikan.moe/v4/seasons/now?sfw=true&page=${page}`);
        ...
        if (error.response && error.response.status === 429) {
          ...
          console.error("Error fetching On-Going anime: ", error);
          setLoading(false);
        }
      }
    };
  ...
  return (
    <>
      <Helmet>
        <title>MaNime - On Going Anime</title>
        <meta property="og:url" content="https://manime-reactjs.vercel.app/ongoing" />
        <meta property="og:title" content="MaNime - On Going Anime" />
        <meta property="og:description" content="Daftar anime yang sedang tayang yang diurutkan berdasarkan tanggal tayang." />
        <meta property="og:image" content="https://manime-reactjs.vercel.app/img/MaNime-white.png" />
        <meta property="og:image:type" content="image/png" />
        <link rel="canonical" href="https://manime-reactjs.vercel.app/ongoing" />
      </Helmet>
      <Header />
      <section className="ongoing-anime2" id="ongoing">
        <h1>On-Going Anime</h1>
        <div className="ongoing-anime2-container">
          {loading
            ? Array.from({ length: onGoingAnime.length > 0 ? onGoingAnime.length : 25 }).map((_, index) => (
                <div className="ongoing-anime2-card" key={index}>
                  <Skeleton height={300} width="100%" />
                </div>
              ))
            : onGoingAnime.map((anime, index) => (
                <div className="ongoing-anime2-card" key={`${anime.mal_id}${index}`}>
                  <Link
                    to={`/detail/${anime.mal_id}/${anime.title
                      ...
                      .replace(/\s+/g, "_")}`}
                  >
                    <img src={anime.images.webp.large_image_url} alt={anime.title} className="ongoing-anime2-image" loading="lazy" />
                    <div className="ongoing-anime2-detail">
