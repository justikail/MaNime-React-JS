...
const Complete = () => {
  ...
  useEffect(() => {
    const fetchCompleteAnime = async () => {
      try {
        const response = await api.get(`https://api.jikan.moe/v4/anime?sfw=true&status=complete&order_by=end_date&sort=desc&limit=25&page=${page}`);
  ...
  return (
    <>
      <Helmet>
        <title>MaNime - Complete Anime</title>
        <meta property="og:url" content="https://manime-reactjs.vercel.app/complete" />
        <meta property="og:title" content="MaNime - Complete Anime" />
        <meta property="og:description" content="Daftar anime yang telah selesai tayang." />
        <meta property="og:image" content="https://manime-reactjs.vercel.app/img/MaNime-white.png" />
        <meta property="og:image:type" content="image/png" />
        <link rel="canonical" href="https://manime-reactjs.vercel.app/complete" />
      </Helmet>
      <Header />
      <section className="complete-anime2" id="ongoing">
        <h1>Complete Anime</h1>
        <div className="complete-anime2-container">
          {loading
            ? Array.from({ length: completeAnime.length > 0 ? completeAnime.length : 25 }).map((_, index) => (
                <div className="complete-anime2-card" key={index}>
                  <Skeleton height={300} width="100%" />
                </div>
              ))
            : completeAnime.map((anime, index) => (
                <div className="complete-anime2-card" key={`${anime.mal_id}${index}`}>
                  <Link
                    to={`/detail/${anime.mal_id}/${anime.title
                      ...
                      .replace(/\s+/g, "_")}`}
                  >
                    <img src={anime.images.webp.large_image_url} alt={anime.title} className="complete-anime2-image" loading="lazy" />
                    <div className="complete-anime2-detail">
